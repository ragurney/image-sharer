...
class ImageTest < ActiveSupport::TestCase
  test 'check url valid' do
    image = Image.new(url: 'https://thisis.com/?stuff.png', tag_list: 'tag')
    assert_predicate image, :valid?
  end

  test 'check url invalid' do
    image = Image.new(url: 'thisis.com/?invalid.png', tag_list: 'tag')
    assert_predicate image, :invalid?
  end

  test 'url invalid with empty string' do
    image = Image.new(url: '', tag_list: 'tag')
    assert_predicate image, :invalid?
  ...
  test 'saved image should be valid with one tag' do
    image = Image.new(url: 'https://valid.com', tag_list: 'tag')
    assert_predicate image, :valid?
  end

  test 'saved image should be invalid with less than one tag' do
    image = Image.new(url: 'https://valid.com', tag_list: '')
    assert_predicate image, :invalid?
